...
if(isset($_SESSION['acceso'])) { 
    if ($_SESSION['acceso'] == "administrador" || $_SESSION['acceso'] == "enfermera" || ($_SESSION['acceso'] == "medico" && in_array("4", explode(",", $_SESSION['modulos'])))) {
...
{
$reg = $tra->RegistrarRadiologias();
exit;
}
elseif(isset($_POST["proceso"]) and $_POST["proceso"]=="update")
{
$reg = $tra->ActualizarRadiologias();
exit;
} 
?>
<!DOCTYPE html>
...
<!--############################## MODAL BUSQUEDA DE CITAS EN RADIOLOGIA ######################################-->
<!-- Modal -->
        ...
        <div class="row">
            <div class="col-md-3">
                <div class="form-group has-feedback">
                    ...
                    <input type="hidden" name="codverifica" id="codverifica" value="<?php echo "4"; ?>"/>
...
<!--############################## MODAL BUSQUEDA DE PLANTILLAS LECTURA RX ######################################-->
<!-- Modal -->
<div class="modal fade" id="myModalPlantilla" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-xl" role="document">
        <div class="modal-content">
            <div class="modal-header bg-primary">
                <h5 class="modal-title text-white" id="exampleModalLabel"><i class="text-white" data-feather="align-justify"></i> Plantillas Lecturas Rx</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <i class="text-white" data-feather="x-circle"></i>
                </button>
            </div>
        ...
        <div class="widget-one">
            ...
            <div class="col-lg-12 filtered-list-search mx-auto">
                <form class="form-inline mb-8 justify-content-center" method="post" action="#" name="plantillaslecturarx" id="plantillaslecturarx">
                    <div class="w-100">
                        <input type="text" class="w-100 form-control product-search br-30" name="blecturarx" id="blecturarx" onKeyUp="this.value=this.value.toUpperCase();" placeholder="Ingrese Criterio para tu Búsqueda" autocomplete="off">
                        <button class="btn btn-primary" type="button" onClick="BuscarPlantillasLecturaRx();"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-search"><circle cx="11" cy="11" r="8"></circle><line x1="21" y1="21" x2="16.65" y2="16.65"></line></svg></button>
                    </div>
                </form>
            </div>
...
<!--############################## MODAL BUSQUEDA DE PLANTILLAS LECTURA RX ######################################-->
                            ...
                            <div class="widget-header">
                                <div class="row">
                                    <div class="col-xl-12 col-md-12 col-sm-12 col-12">
                                        <h5 class="card-subtitle text-dark alert-link"><i data-feather="align-justify"></i> Gestión de Radiología</h5>
                                    </div>                 
                                </div>
                            </div>
    ...
    <form class="form-material" novalidate method="post" action="#" name="updateradiologia" id="updateradiologia" data-id="<?php echo $reg[0]["codradiologia"] ?>" enctype="multipart/form-data">
        
    <?php } else { ?>

    <form class="form-material" novalidate method="post" action="#" name="saveradiologia" id="saveradiologia" enctype="multipart/form-data">
    ...
    <?php } ?>
        ...
        <div class="row">
            <div class="col-md-3">
                ...
                <div class="input-group">
                    <input type="hidden" name="proceso" id="proceso" <?php if (isset($reg[0]['codradiologia'])) { ?> value="update" <?php } else { ?>  value="save" <?php } ?>/>
                    <input type="hidden" name="idradiologia" id="idradiologia" <?php if (isset($reg[0]['idradiologia'])) { ?> value="<?php echo encrypt($reg[0]['idradiologia']); ?>" <?php } ?>>
                    <input type="hidden" name="codradiologia" id="codradiologia" <?php if (isset($reg[0]['codradiologia'])) { ?> value="<?php echo encrypt($reg[0]['codradiologia']); ?>" <?php } ?>/>
                    ...
                    <?php if (isset($reg[0]['idradiologia'])) { ?>
        ...
        <hr><h5 class="card-subtitle text-dark alert-link"><i data-feather="file-text"></i> Procedimientos para Lectura Rx</h5><hr>

        <div class="row">
            <div class="col-md-2">
                <div class="form-group">
                    <label class="control-label">Realizar Lectura: <span class="symbol required"></span></label>
                    <br>
                    <div class="custom-control custom-radio custom-control-inline">
                        <input type="radio" id="SI" name="lectura" value="1" onClick="LecturaRx();" <?php if (isset($reg[0]['lectura']) && $reg[0]['lectura'] == 1) { ?> checked="checked" <?php } ?> class="custom-control-input">
                        <label class="custom-control-label text-dark alert-link" for="SI">SI</label>
                    </div><br>
                    <div class="custom-control custom-radio custom-control-inline">
                        <?php if (isset($reg[0]['lectura'])) { ?>
                        <input type="radio" id="NO" name="lectura" value="2" onClick="LecturaRx();" <?php if ($reg[0]['lectura'] == 2) { ?> checked="checked" <?php } ?> class="custom-control-input">
                        <?php } else { ?> 
                        <input type="radio" id="NO" name="lectura" value="2" onClick="LecturaRx();" checked="checked" class="custom-control-input">
                        <?php } ?>
            ...
            <div class="col-md-10">
                ...
                <div class="input-group">
                    ...
                    <?php if (isset($reg[0]['tipoestudio'])) { ?>
                    <input type="text" class="form-control" name="tipoestudio" id="tipoestudio" onKeyUp="this.value=this.value.toUpperCase();" placeholder="Ingrese Tipo de Estudio" autocomplete="off" 
                    <?php if ($reg[0]['lectura'] == 1) { ?> value="<?php echo $lectura = ($reg[0]['lectura'] == 1 ? $reg[0]['tipoestudio'] : ""); ?>" <?php } else { ?> disabled="" <?php } ?> required="" aria-required="true"/>
                    <?php } else { ?> 
                    <input type="text" class="form-control" name="tipoestudio" id="tipoestudio" onKeyUp="this.value=this.value.toUpperCase();" placeholder="Ingrese Tipo de Estudio" autocomplete="off" disabled=""  required="" aria-required="true"/>
                    <?php } ?>

                    <div class="input-group-append">
                        <div class="btn-group" data-bs-toggle="buttons">
                            <button type="button" id="BotonPlantilla" onclick="MostrarPlantillasLecturaRx();" class="btn btn-primary waves-effect waves-light" data-placement="left" title="Buscar Plantilla Lectura Rx" data-original-title="" data-href="#" data-toggle="modal" data-target="#myModalPlantilla" data-backdrop="static" data-keyboard="false" disabled=""><i data-feather="file-plus"></i>Buscar Plantilla</button>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="row">
            <div class="col-md-12"> 
                <div class="form-group has-feedback2"> 
                    ...
                    <?php if (isset($reg[0]['diagnostico'])) { ?>
                    <textarea class="form-control" type="text" name="diagnostico" id="diagnostico" onKeyUp="this.value=this.value.toUpperCase();" autocomplete="off" placeholder="Ingrese Diagnóstico de Lectura RX" rows="12" <?php if ($reg[0]['lectura'] == 1) { ?>  <?php } else { ?> disabled="" <?php } ?>><?php echo $lectura = ($reg[0]['lectura'] == 1 ? $reg[0]['diagnostico'] : ""); ?></textarea>
                    <?php } else { ?> 
                    <textarea class="form-control" type="text" name="diagnostico" id="diagnostico" onKeyUp="this.value=this.value.toUpperCase();" autocomplete="off" placeholder="Ingrese Diagnóstico de Lectura RX" rows="12" disabled=""></textarea>
                    <?php } ?>
                </div> 
            </div>
        </div>
...
<button class="btn btn-dark" type="reset"><i data-feather="trash-2"></i> Limpiar</button>
    ...
    <script type="text/javascript">
    $('#blecturarx').keypress(function(e) {
    var keycode = (e.keyCode ? e.keyCode : e.which);
        if (keycode == '13') {
            BuscarPlantillasLecturaRx();
            return false;
        }
    });   
    </script>
